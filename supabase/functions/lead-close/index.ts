// Supabase Edge Function: lead-close
// - Cambia un lead a estado CERRADO.
...
async function highLevelUpdateOpportunityStage(
  ...
  // La API ha mostrado ser sensible por método/shape; intentamos variantes sin exponer secretos.
  ...
  for (const locationQueryKey of locationQueryKeys) {
    for (const path of paths) {
      for (const body of payloads) {
        ...
        console.log(
          `[lead-close] LeadConnector PUT ${path} (q=${locationQueryKey ?? "none"}) -> ${res.status}`,
          res.body,
        );
...
serve(async (req) => {
  ...
  const hlStageCerrado = Deno.env.get("HIGHLEVEL_STAGE_ID_CERRADO");
  ...
  if (!hlStageCerrado) hlMissing.push("HIGHLEVEL_STAGE_ID_CERRADO");
  ...
  if (lead.hl_status !== "CREATED") {
    return jsonResponse(400, { error: "Solo se pueden cerrar leads con hl_status=CREATED." });
  }
  ...
  if (lead.lead_state !== "REUNION") {
    return jsonResponse(400, { error: "Solo se puede cerrar una venta desde estado REUNION." });
  }
  ...
  const { data: updated, error: upErr } = await adminClient
    ...
    .eq("id", leadId)
    .select("id, lead_state, closed_at, hl_opportunity_id")
    .single();
  ...
  if (updated.hl_opportunity_id) {
    ...
    console.log("[lead-close] Lead sin hl_opportunity_id; se omite update de stage.");
  }
