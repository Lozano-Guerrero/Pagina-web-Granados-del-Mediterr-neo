...
const HeroSlider = () => {
        ...
        // useEffect para manejar la visibilidad de la página y resincronizar el slider
        useEffect(() => {
                const handleVisibilityChange = () => {
                        if (document.visibilityState === 'visible') {
                                // Al volver, forzamos un estado limpio
                                setIsTransitioning(false);
                                setReloadTrigger(prev => prev + 1);
                                // Si el slider estaba en un clon o perdido, lo devolvemos al inicio real
                                if (currentSlideIndex === 0 || currentSlideIndex >= totalExtendedSlides - 1) {
                                        setCurrentSlideIndex(1);
                                }
                        } else {
                                // Al salir, pausamos el intervalo limpiándolo
                                if (intervalRef.current) clearInterval(intervalRef.current);
                        }
                };
                document.addEventListener('visibilitychange', handleVisibilityChange);
                return () => document.removeEventListener('visibilitychange', handleVisibilityChange);
        }, [currentSlideIndex, totalExtendedSlides]);
        ...
        // Lógica para el cambio automático de diapositivas con protección de visibilidad
        useEffect(() => {
                // Solo iniciar si la página es visible
                if (document.visibilityState === 'visible') {
                        startAutoSlide();
                }
        ...
        }, [startAutoSlide, reloadTrigger]); // Reacciona al volver a la pestaña
